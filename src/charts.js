...
const DARK = { paper_bgcolor: 'rgba(0,0,0,0)', plot_bgcolor: 'rgba(0,0,0,0)', font: { color: '#8b98b0', family: 'Inter, system-ui, sans-serif' }, margin: { t: 30, r: 10, b: 50, l: 60 } };
const AXIS = { gridcolor: '#22304d', zerolinecolor: '#22304d', linecolor: '#22304d' };
...
export function makeCharts(Plotly) {
  ...
  return {
    bar(id, top) {
      ...
      plot(id, [{ type: 'bar', orientation: 'h', x: rows.map((o) => o.value), y: rows.map((o) => `OTU ${o.id}`), text: rows.map((o) => o.short), hovertemplate: '%{y}: %{x} reads<br>%{text}<extra></extra>', marker: { color: '#58A6FF' } }], { xaxis: { title: 'Reads', ...AXIS }, yaxis: { automargin: true, ...AXIS }, margin: { t: 10, r: 10, b: 50, l: 90 } });
    },
    bubble(id, all) {
    ...
    pie(id, top) {
      plot(id, [{ type: 'pie', values: top.map((o) => o.value), labels: top.map((o) => `OTU ${o.id}`), text: top.map((o) => o.short), hovertemplate: '%{label}: %{value} reads (%{percent})<br>%{text}<extra></extra>', textinfo: 'percent', marker: { colors: ['#58A6FF', '#3fb950', '#d29922', '#f85149', '#d2a8ff'] } }], { showlegend: true, legend: { font: { color: '#8b98b0' } }, margin: { t: 10, r: 10, b: 10, l: 10 } });
    },
    ...
    gauge(svg, fraction, label) {
      ...
      arc(1, '#22304d', 14);
      if (fraction !== null && fraction > 0) arc(fraction, fraction < 0.34 ? '#f85149' : fraction < 0.67 ? '#d29922' : '#3fb950', 14);
